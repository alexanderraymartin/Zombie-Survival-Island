...
public class Gun : NetworkBehaviour
{
    ...
    public void Shoot()
    {
        ...
        // Play shooting sound
        gunOwner.soundManager.PlaySound(gunOwner.connectionId, shootingSoundIndex, transform.position, 0.15f);

        // Show muzzle flash
        gunOwner.weaponManager.MuzzleFlash(gunOwner.connectionId);
        ...
        {
            gunOwner.weaponManager.HitEffect(gunOwner.connectionId, hits[i].point, hits[i].normal);
            if (hits[i].transform.gameObject.tag == "Enemy")
            {
                float calculatedDamage = damage * (Mathf.Pow(bulletPenetration / 100, i));
                gunOwner.weaponManager.DealDamage(gunOwner.connectionId, hits[i].transform.gameObject, calculatedDamage);
    ...
    public IEnumerator Reload()
    {
        ...
        gunOwner.soundManager.PlaySound(gunOwner.connectionId, reloadingSoundIndex, transform.position, 0.15f);
